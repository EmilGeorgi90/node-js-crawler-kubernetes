...
const written = new client.Counter({
  name: "collector_s3_written_total",
  help: "raw records written",
  registers: [metricsRegistry],
});
startMetricsServer(metricsRegistry, Number(process.env.METRICS_PORT || "9103"));
...
(async () => {
  ...
  await kafkaBus.runConsumer({
    topics: [appConfig.TOPIC_RESULTS_RAW],
    groupId: appConfig.GROUP_RAW,
    concurrency: 3,
    handler: async (buf) => {
