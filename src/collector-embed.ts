...
const embeddedPages = new client.Counter({
  ...
  help: 'Pages embedded and written to pgvector',
});
const embedFailures = new client.Counter({
  name: 'collector_embed_failures_total',
  help: 'Embedding or write failures',
});
const embedLatency = new client.Histogram({
  name: 'collector_embed_duration_seconds',
  help: 'Embedding + write duration (per batch item)',
  buckets: [0.02,0.05,0.1,0.25,0.5,1,2,5],
});
registry.registerMetric(embeddedPages);
registry.registerMetric(embedFailures);
registry.registerMetric(embedLatency);
...
const EMBEDDER_URL = process.env.EMBEDDER_URL || 'http://embedder:8000/embed';
const BATCH_SIZE   = Number(process.env.EMBED_BATCH || '32');
const VECTOR_DIM   = Math.max(1, Math.min(4096, Number(process.env.VECTOR_DIM || '384')));
const INLINE_VECTOR_SQL = (process.env.INLINE_VECTOR_SQL ?? 'true').toLowerCase() !== 'false';

function toText(b: any): string {
  const title = b?.title ? String(b.title) : '';
  let bodyText = '';
  try {
    const collect = (n: any) => { if (!n) return; if (typeof n.text === 'string') bodyText += ' ' + n.text; if (Array.isArray(n.children)) n.children.forEach(collect); };
...
async function embedLocal(texts: string[]) {
  const resp = await fetch(EMBEDDER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ input: texts }) });
  if (!resp.ok) throw new Error(`Embedder ${resp.status}: ${await resp.text()}`);
  const j: any = await resp.json();
  return j.data as number[][];
}

function toArrayCast(v: number[], dim: number): string {
  return `ARRAY[${v.map(x => (Number.isFinite(x) ? x : 0)).join(',')}]::vector(${dim})`;
}
...
(async () => {
  ...
  const pg = new PgClient({ host: PG_HOST, port: PG_PORT, database: PG_DB, user: PG_USER, password: PG_PASS });
  await pg.connect();
  ...
  while (true) {
    try {
      const batch = await pages.find({ 'meta.isEmbedded': { $ne: true } }).project({ url: 1, title: 1, detectedType: 1, body: 1 }).limit(BATCH_SIZE).toArray();
      ...
      for (let i = 0; i < batch.length; i++) {
        const b = batch[i]; const v = vecs[i];
        if (!Array.isArray(v) || v.length !== VECTOR_DIM) throw new Error(`dim ${v?.length} != ${VECTOR_DIM} for ${b.url}`);
      ...
      await pages.updateMany({ _id: { $in: ids } }, { $set: { 'meta.isEmbedded': true, 'meta.embeddedAt': new Date() } });
